...
export default class Search extends Component {
  ...
  componentDidMount = async () => {
    ...
    let books = await axios.get(
      `http://localhost:4000/books/search/${keyword}/${tag}`
    );
    ...
    return (
      <>
        ...
        <SearchStyle>
          ...
          <h4>
            ( Searched keyword: "
            {keyword === "keyword" ? "No Keyword Provided" : keyword}", searched
            by tag: "{tag === "tag" ? "No Tag Provided" : tag}")
          </h4>
          {this.state.books.length > 1 ? (
            ...
            "No books by that search combination."
          )}
        </SearchStyle>
        <Footer />
      </>
